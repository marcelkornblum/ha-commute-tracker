...
8. **Legacy PoC Baseline & UI Parity**: A working, highly effective (though unoptimised) proof of concept is located in `conductor/legacy_poc/` for direct reference. The Python specification and implementation are intended to largely replicate that working structure in pure Python, replacing ad-hoc API queries with the decoupled `TransitProvider` plugin architecture and optimising intermediate calculations. Crucially, the original UI design was proven and effective—the custom Lovelace card must achieve exact visual parity and must not look different.
